...
const mappingEntries = normalizeTimedStepMapping({
...
describe('buildTimedSteps', () => {
  test('empty input returns empty array', () => {
    const result = buildTimedSteps([], mappingEntries)
  ...
  test('skips unmapped types', () => {
    ...
    const result = buildTimedSteps(files, mappingEntries)
    expect(result).toHaveLength(0)
  })
  ...
  test('multiple files for same testcase at same time+type are grouped', () => {
    ...
    const result = buildTimedSteps(files, mappingEntries)
    expect(result).toHaveLength(1)
    expect(result[0].data.TC_01.files).toHaveLength(2)
  })
