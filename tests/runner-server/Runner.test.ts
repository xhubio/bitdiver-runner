...
test(
  ...
  async () => {
    ...
    const stepName = suiteDefiniton.steps[options.posStep]
    suiteDefiniton.testcases[options.posTc].data[stepName] = data
...
test(
  ...
  async () => {
    ...
    suiteDefiniton.stepDefinitions[runOnErrorStep.name] = runOnErrorStep
    suiteDefiniton.steps.push(runOnErrorStep.name)
...
test(
  ...
  async () => {
    ...
    expect(logs['TC 1'].steps['Step single which runs after Error'].logs[2]).toEqual({
      countAll: 6,
      countCurrent: 6,
      data: { message: 'Step run' },
      logLevel: 'info'
 ...
 */
async function runTestcaseHasStatusWarning(opts = {}): Promise<CheckTcStatusResult> {
 ...
 */
async function runTestcaseHasStatusError(opts = {}): Promise<CheckTcStatusResult> {
 ...
 */
async function runTestcaseHasStatusException(opts = {}): Promise<CheckTcStatusResult> {
