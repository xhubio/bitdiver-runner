 ...
 */
export function createSuite(opts: CreateSuiteOptions): SuiteDefinitionInterface {
  const param = {
  ...
  for (let i = 0; i < param.stepCount; i++) {
    ...
    suite.stepDefinitions[step.name] = step
    suite.steps.push(step.name)
  }
  ...
  for (let i = 0; i < param.testcaseCount; i++) {
    ...
    const data: { [stepName: string]: any } = {}
    for (let j = 0; j < suite.steps.length; j++) {
      const stepName = suite.steps[j]
      data[stepName] = { tc: i + 1, step: j + 1 }
    }
